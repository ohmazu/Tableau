...
### LOD 표현식
- 현재 뷰에는 영향받지 않고 본인이 원하는 세부 수준에서 제어 가능
...
```
FIXED는 특정 차원을 사용해 계산하기 때문에 필터의 영향을 받지 않지만, EXCLUDE는 뷰에 있는 차원을 따라 계산하기 때문에 관련 차원을 필터로 걸어보면 필터가 영향을 받음
```
### EXCLUDE LOD
...
```
특정 차원이 여러 값을 가질 때 단일 값으로 표현하기 위한 함수
```
...
- 매개변수(Parameter)는 사용자가 동적으로 값을 입력할 수 있는 컨트롤 요소
- 필터, 계산된 필드, 집합 등 다양한 곳에 연결해 동적 시각화를 구현할 수 있음

### 매개변수
- 동적인 값으로 변경하기 위해서 활용하는 기능으로 매개변수는 반드시 계산식, 필터, 참조선과 함께 사용
...
## 45. 워크시트 마크카드
- 마크카드는 태블로에서 데이터의 표현 방식을 설정하는 도구
- 색상, 크기, 텍스트, 툴팁, 모양 등 다양한 속성에 필드를 드래그하여 시각화에 적용 가능
- 시각화의 의미 전달력을 높이는 데 핵심적인 역할
...
- 테두리 유형, 두께, 색사
- 서식을 일관적이게 적용하면 시각화의 가독성과 전달력 향상
